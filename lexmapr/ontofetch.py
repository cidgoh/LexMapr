...
class Ontology(object):
    ...
    def __init__(self):
        ...
        self.fields = ['id', 'parent_id', 'language', 'ontology', 'other_parents', 'label',
                       'definition', 'ul_label', 'ui_definition', 'ui_help', 'synonyms',
    ...
    def __main__(self):
        ...
        print('Metadata: ' + json.dumps(self.onto_helper.struct['metadata'], sort_keys=False,
                                        indent=4, separators=(',', ': ')))
    ...
    def do_entities(self, table):
        ...
        for parent_id in parents:
            if not parent_id in self.onto_helper.struct['specifications']:
                self.onto_helper.set_entity_default(self.onto_helper.struct, 'specifications',
                                                    parent_id, {
                                                        'id': parent_id,
                                                        'datatype': 'entity'
                                                    })
    ...
    def do_entity_synonyms(self, id):
        ...
        myURI = rdflib.URIRef(self.onto_helper.get_expanded_id(id))
        rows = self.onto_helper.graph.query(self.queries['entity_synonyms'],
                                            initBindings={'datum': myURI})
        synonymArray = []
        ...
        if len(synonymArray) > 0:
            ...
            self.onto_helper.set_entity_default(self.onto_helper.struct, 'specifications', id,
    ...
    def get_command_line(self):
		...
		*************************** Parse Command Line *****************************
        ...
        parser.add_option('-r', '--root', dest='root_uri', type='string',
                          help='Root term to fetch underlying terms from (full URI)',
