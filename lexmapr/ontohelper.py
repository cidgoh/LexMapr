...
class OntoHelper(object):
    ...
    def get_struct(self, focus, *args):
        ...
        try:
            ...
            print("ERROR: in get_struct(), couldn't find '%s' key or struct in %s" % (
            str(arg), str(args)))
            return None
        return focus
    ...
    def set_entity_default(self, focus, *args):
        ...
        for ptr, arg in enumerate(args[0:-1]):
            ...
            if not arg: stop_err(
                "ERROR: in set_entity_default(), an argument isn't set: %s" % str(args[0:-1]))
            if ptr == len(args) - 2:
                if not arg in focus:
                    focus[arg] = value
                return focus[arg]
    ...
    def reorder(self, entity, part, orderedKeys=None):
        ...
        if part in entity:
            if orderedKeys:
                ...
                entity[part] = OrderedDict(sorted(entity[part].items(),
                                                  key=lambda item: orderedKeys.index(item[0]) if
                                                  item[0] in orderedKeys else False))
            else:
    ...
    def do_ontology_includes(self, main_ontology_file):
        ...
        for result_row in imports:
            ...
            # since, as a local resource, its imports should be local too.
                ...
                file_path = os.path.dirname(main_ontology_file) + '/imports/' + \
                            import_file.rsplit('/', 1)[1]
                ...
                try:
                    if os.path.isfile(file_path):
                        ...
                        print(
                            'WARNING:' + file_path + " could not be loaded!  Does its ontology include purl have a corresponding local file? \n")
        ...
        for ptr, row in enumerate(result):
            ...
            for column in rowdict:
                ...
                if valType is rdflib.term.URIRef:
                    ...
                    newrowdict[column] = {
                        'value': 'unrecognized column [%s] type %s for value %s' % (
    ...
    def do_output_json(self, struct, output_file_basename):
        with (open(output_file_basename + '.json', 'w')) as output_handle:
            ...
            output_handle.write(
                json.dumps(struct, sort_keys=False, indent=4, separators=(',', ': ')))
        ...
        for (key, entity) in struct['specifications'].items():
            ...
            for field in fields:
                ...
                if isinstance(value, list):  # Constructed parent_id list.
                    value = ','.join(value)
                if sys.version_info[0] < 3:  # Python 2
                    row.append(value.replace('\t', ' ').encode(
                        'utf-8'))  # str() handles other_parents array
                else:  # Python 3
